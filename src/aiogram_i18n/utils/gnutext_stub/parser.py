...
def parse_po_file(file: Path) -> Messages:
    try:
        return parse(cast(list[POEntry], pofile(file.as_posix())))
    except ValueError as e:
        red(f"file {file} {e.args[0]}")
        raise


def parse_mo_file(file: Path) -> Messages:
    try:
        return parse(cast(list[MOEntry], mofile(file.as_posix())))
    except ValueError as e:
        red(f"file {file} {e.args[0]}")
        raise
